...
Goal: find concrete places where the code under review wastes computation the user waits on, name the cost class, and propose the specific fix — not a generic call to "optimize."
...
- Flag only waste that costs real time at realistic input sizes. A quadratic pass over a list that is always three elements is not a finding.
...
For each item, look for the wasteful pattern itself, not just for names or keywords that hint at it.
...
2. Walk the checklist above against the code under review. For each hit, record: the file:line, which item it fails, a one-sentence statement of the concrete failure mode (not a restatement of the item), the cost class it changes (e.g. O(n²)→O(n), N+1→1 query), and a specific proposed fix.
