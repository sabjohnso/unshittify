...
Goal: confirm that the production code under review was demanded by a failing test written first, that the implementation is no more than the tests require, and that anything labeled a refactor left test assertions unchanged.

## Checklist

1. **Red before green** — a commit (or hunk) that adds or changes production code with no preceding commit adding a test that would have failed against the old code. Order matters: a test added in the same commit as the implementation it exercises is acceptable only if the project's commit history shows the same pattern elsewhere; a test added *after* the implementation it exercises is not TDD regardless of commit grouping.
...
4. **Passing tests as evidence, not assumed correctness** — flag any claim in a commit message or comment that code is "correct" or "complete" unless the commit references or reproduces the corresponding test run's output.
