...
- Frontmatter fields, in this order:
  ...
  - `disable-model-invocation` (optional, bool) — set `true` for skills that perform a consequential or scoped action (changing settings) that should only run when the user explicitly types the slash command, never inferred from conversation. Omit it for skills that are safe to auto-trigger from the description alone, including consequential actions already gated by an explicit-instruction requirement enforced elsewhere (e.g. `plugins/git/skills/commit-message/SKILL.md` omits it deliberately: committing is already restricted to explicit user requests by standing instruction, and `hooks/confirm-git-commit-push.sh` confirms before the underlying `git commit` runs, so gating the skill itself behind the literal slash command would only make it easier to bypass with a freehand message).
